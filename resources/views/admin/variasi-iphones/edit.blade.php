...
<div class="container-fluid px-4">
    <div class="card">
        <div class="card-header">
            <h3>Edit Variasi iPhone</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('variasi-iphones.update', $variasiIphone) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <select class="form-select @error('iphone_id') is-invalid @enderror" 
                        ...
                        @foreach($iphones as $iphone)
                            <option value="{{ $iphone->id }}" 
                                {{ (old('iphone_id', $variasiIphone->iphone_id) == $iphone->id) ? 'selected' : '' }}>
                                {{ $iphone->nama }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select class="form-select @error('warna_id') is-invalid @enderror" 
                        ...
                        @foreach($warnas as $warna)
                            <option value="{{ $warna->id }}" 
                                {{ (old('warna_id', $variasiIphone->warna_id) == $warna->id) ? 'selected' : '' }}>
                                {{ $warna->nama_warna }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select class="form-select @error('penyimpanan_id') is-invalid @enderror" 
                        ...
                        @foreach($penyimpanans as $penyimpanan)
                            <option value="{{ $penyimpanan->id }}" 
                                {{ (old('penyimpanan_id', $variasiIphone->penyimpanan_id) == $penyimpanan->id) ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control @error('harga_sewa_per_hari') is-invalid @enderror" 
                               ...
                               value="{{ old('harga_sewa_per_hari', $variasiIphone->harga_sewa_per_hari) }}" 
                               required min="0">
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('stok') is-invalid @enderror" 
                           id="stok" name="stok" 
                           value="{{ old('stok', $variasiIphone->stok) }}" required min="0">
                    @error('stok')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="tersedia" 
                               ...
                               {{ old('tersedia', $variasiIphone->tersedia) ? 'checked' : '' }}>
